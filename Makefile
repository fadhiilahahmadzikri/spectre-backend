...
install: ## Install all dependencies (production + dev)
	Set-Location spectre-backend; uv sync --all-extras

dev: ## Run development server (Unix)
	Set-Location spectre-backend; $$env:PYTHONDONTWRITEBYTECODE='1'; uv run uvicorn spectre.main:create_app --factory --host 0.0.0.0 --port 8000 --reload

dev-win: clean-pyc ## Run development server (Windows, auto-clears cache)
	Set-Location spectre-backend; $$env:PYTHONDONTWRITEBYTECODE='1'; .venv\Scripts\python -m uvicorn "src.spectre.main:create_app" --factory --host 0.0.0.0 --port 8000 --reload
...
test-poc: ## Serve POC HTML UI client
	...
	@echo "========================================================"
	spectre-backend\.venv\Scripts\python -m http.server 8001 -d poc

worker: ## Run Celery worker
	Set-Location spectre-backend; uv run celery -A spectre.workers.celery_app worker --loglevel=info --concurrency=2
...
lint: ## Run linter
	Set-Location spectre-backend; uv run ruff check src/ tests/

format: ## Format code
	Set-Location spectre-backend; uv run ruff format src/ tests/

typecheck: ## Run type checker
	Set-Location spectre-backend; uv run mypy src/spectre
...
test: ## Run all pytest tests
	Set-Location spectre-backend; uv run pytest

test-unit: ## Run unit tests only
	Set-Location spectre-backend; uv run pytest tests/unit -v

test-integration: ## Run integration tests only
	Set-Location spectre-backend; uv run pytest tests/integration -v

test-cov: ## Run tests with coverage report
	Set-Location spectre-backend; uv run pytest --cov=spectre --cov-report=html --cov-report=term-missing

test-newman: ## Run Newman API tests against local server
	Set-Location spectre-backend; newman run tests/postman/spectre-api-v1.postman_collection.json -e tests/postman/env-local.json --reporters cli

test-newman-hf: ## Run Newman API tests against Hugging Face Spaces
	Set-Location spectre-backend; newman run tests/postman/spectre-api-v1.postman_collection.json -e tests/postman/env-hf-spaces.json --reporters cli --timeout-request 30000

test-newman-report: ## Run Newman with HTML report (local)
	Set-Location spectre-backend; New-Item -ItemType Directory -Force -Path reports | Out-Null
	Set-Location spectre-backend; newman run tests/postman/spectre-api-v1.postman_collection.json -e tests/postman/env-local.json --reporters cli,htmlextra --reporter-htmlextra-export reports/api-report.html
	@echo "Report saved: spectre-backend/reports/api-report.html"

test-newman-hf-report: ## Run Newman with HTML report (HF Spaces)
	Set-Location spectre-backend; New-Item -ItemType Directory -Force -Path reports | Out-Null
	Set-Location spectre-backend; newman run tests/postman/spectre-api-v1.postman_collection.json -e tests/postman/env-hf-spaces.json --reporters cli,htmlextra --reporter-htmlextra-export reports/api-report-hf.html --timeout-request 30000
	@echo "Report saved: spectre-backend/reports/api-report-hf.html"

postman-push: ## Push collection to Postman cloud (syncs local → remote)
	Set-Location spectre-backend; uv run python scripts/postman_push.py
...
migrate: ## Run database migrations to latest (local)
	Set-Location spectre-backend; uv run alembic upgrade head

migrate-supabase: ## Run database migrations against Supabase (production)
	Set-Location spectre-backend; $$env:DATABASE_URL = (Select-String -Path .env.spaces -Pattern '^DATABASE_URL=(.+)$$' | ForEach-Object { $$_.Matches.Groups[1].Value }); uv run alembic upgrade head

migrate-new: ## Create new migration (usage: make migrate-new MSG="description")
	Set-Location spectre-backend; uv run alembic revision --autogenerate -m "$(MSG)"

migrate-down: ## Rollback one migration
	Set-Location spectre-backend; uv run alembic downgrade -1
...
seed: ## Run all database seeders
	Set-Location spectre-backend; uv run python -m seeds all

seed-force: ## Force re-run all seeders (ignore idempotency)
	Set-Location spectre-backend; uv run python -m seeds all --force

seed-reset: ## Clear seed registry so seeders can re-run
	Set-Location spectre-backend; uv run python -m seeds reset --yes

seed-list: ## List all registered seeders with status
	Set-Location spectre-backend; uv run python -m seeds list
...
docker-up: ## Start infra services only (PostgreSQL, Redis)
	Set-Location spectre-backend; docker-compose up -d

docker-down: ## Stop all services
	Set-Location spectre-backend; docker-compose down

docker-build: ## Build Docker images
	Set-Location spectre-backend; docker-compose build

docker-logs: ## Tail service logs
	Set-Location spectre-backend; docker-compose logs -f

docker-dev: ## Start FULL containerized dev stack (zero local deps)
	Set-Location spectre-backend; docker compose -f docker-compose.dev.yml up --build

docker-dev-d: ## Start full dev stack (detached)
	Set-Location spectre-backend; docker compose -f docker-compose.dev.yml up --build -d

docker-dev-down: ## Stop full dev stack
	Set-Location spectre-backend; docker compose -f docker-compose.dev.yml down

docker-dev-logs: ## Tail full dev stack logs
	Set-Location spectre-backend; docker compose -f docker-compose.dev.yml logs -f

docker-test: ## Run Newman API tests inside Docker (full stack must be running)
	Set-Location spectre-backend; docker compose -f docker-compose.dev.yml --profile test run --rm newman

docker-reset: ## Full Docker reset (remove volumes, rebuild)
	Set-Location spectre-backend; docker compose -f docker-compose.dev.yml down -v
	Set-Location spectre-backend; docker compose -f docker-compose.dev.yml up --build -d
...
env-check: ## Validate all development dependencies are installed
	Set-Location spectre-backend; powershell -ExecutionPolicy Bypass -File scripts/audit-test-deps.ps1
...
infra-status: ## Show status of HF Spaces and Supabase projects
	Set-Location spectre-backend; python deploy.py --inspect
	Set-Location spectre-backend; npx supabase projects list
...
clean: ## Remove all build artifacts and caches
	...
	Remove-Item -Force spectre-backend/.coverage -ErrorAction SilentlyContinue
