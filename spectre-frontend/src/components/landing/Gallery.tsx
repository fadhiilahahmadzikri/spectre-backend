...
function Card({ src, title, description, stretch = false }: CardProps) {
  return (
    <div className={`relative rounded-[18px] overflow-hidden bg-neutral-surface w-full ${stretch ? 'h-full' : ''}`}>
      ...
      <div className="absolute inset-x-0 bottom-0 px-5 pb-5">
        <p className="text-white text-xl sm:text-2xl font-normal leading-tight font-serif mb-2">
          {title}
        </p>
...
const ALL_CARDS = [...left, ...center, ...right]
...
function MobileSlideshow() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setVisible(false)
      setTimeout(() => {
        setCurrent(i => (i + 1) % ALL_CARDS.length)
        setVisible(true)
      }, 400)
    }, INTERVAL_MS)
  ...
  return (
    <div className="md:hidden">
      ...
      <div className="flex items-center justify-center gap-3 py-4 mb-4">
        <MaskIcon tilt="left" />
        <p className="text-neutral-ink text-xl font-normal font-serif text-center">
          Reality Can Be Deceived
        </p>
        <MaskIcon tilt="right" />
      </div>

      {/* Single card dengan fade — full width, tanpa rounded */}
      <div
        className="relative overflow-hidden bg-neutral-surface w-screen aspect-[4/3]"
        style={{
      ...
      >
        <img
          key={item.src}
          src={`/assets_compressed/${item.src}`}
          alt={item.title}
          className="w-full h-full object-cover"
          loading="lazy"
        />
        ...
        <div
          className="absolute inset-x-0 bottom-0 pointer-events-none"
          style={{ top: '20%', background: 'linear-gradient(to bottom, rgba(217,217,217,0) 0%, rgba(4,4,4,0.49) 49%, #040404 100%)' }}
        />
        {/* Caption */}
        <div className="absolute inset-x-0 bottom-0 px-5 pb-5">
          <p className="text-white text-xl font-normal leading-tight font-serif mb-1">{item.title}</p>
  ...
  return (
    <section className="py-12 sm:py-24 bg-neutral-canvas">
      <div className="max-w-[1706px] mx-auto px-8">
        ...
        {/* Mobile: slideshow dengan fade transition */}
        <MobileSlideshow />

        {/* Desktop: 3-column asymmetric grid */}
        <div
          className="hidden md:grid gap-[22px] items-stretch"
        ...
        >
          ...
          <div className="flex flex-col gap-[22px] h-full">
            ...
            <div className="flex items-center justify-center gap-3 py-2 shrink-0">
              <MaskIcon tilt="left" />
              <p className="text-neutral-ink text-2xl lg:text-4xl font-normal font-serif whitespace-nowrap">
                Reality Can Be Deceived
              </p>
              <MaskIcon tilt="right" />
            </div>
