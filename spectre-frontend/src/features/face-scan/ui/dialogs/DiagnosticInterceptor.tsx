...
export function DiagnosticInterceptor({ open, diagnostics, benchmark, onContinue }: Props) {
  ...
  return (
    <GlassDialog open={open} onOpenChange={() => {}} size="lg" showClose={false} className="h-[88vh]">
      ...
      <GlassDialogBody>
        {hasBoth && (
          <div className="flex gap-1 p-1 rounded-xl bg-[color:var(--surface)] border border-[color:var(--separator)] mb-4 shrink-0">
...
function TabBtn({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
    <button onClick={onClick} className={`flex-1 text-[11px] font-medium px-3 py-1.5 rounded-lg transition-all inline-flex items-center justify-center gap-1.5 ${active ? "bg-[color:var(--surface)] text-[color:var(--label-primary)]" : "text-[color:var(--label-tertiary)] hover:text-[color:var(--label-primary)]"}`}>
      {icon}{label}
    </button>
  );
}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className={`flex items-center gap-2 p-3 rounded-lg ${o.tone === "ok" ? "bg-emerald-500/10 border border-emerald-500/30" : o.tone === "err" ? "bg-red-500/10 border border-red-500/30" : "bg-amber-500/10 border border-amber-500/30"}`}>
        ...
        <span className="text-[12px] font-semibold text-[color:var(--label-primary)]">{o.text}</span>
      ...
      <details className="rounded-lg border border-[color:var(--separator)] p-2"><summary className="cursor-pointer text-[9px] font-semibold text-[color:var(--label-tertiary)] uppercase">Raw JSON</summary><pre className="kbd-mono text-[9px] text-[color:var(--label-tertiary)] mt-1 overflow-auto max-h-32">{JSON.stringify(d,null,2)}</pre></details>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {r.results.map((m) => {
        ...
        return (
          <div key={m.model_id} className="rounded-lg border border-[color:var(--separator)] bg-[color:var(--surface)] p-3 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2"><Cpu size={12} className="text-[color:var(--label-tertiary)]" /><span className="text-[12px] font-semibold text-[color:var(--label-primary)]">{m.model_id}</span><span className="kbd-mono text-[9px] text-[color:var(--label-tertiary)]">v{m.version}</span></div>
...
function ProbBar({ cls, prob, isTop }: { cls: string; prob: number; isTop: boolean }) {
  return (
    <div className="flex items-center gap-1.5">
      <span className={`text-[9px] w-24 ${isTop?"text-[color:var(--label-primary)] font-medium":"text-[color:var(--label-tertiary)]"}`}>{cls}</span>
      <div className="flex-1 h-1.5 rounded-full bg-[color:var(--surface)] overflow-hidden"><div className={`h-full rounded-full ${cls==="realperson"?"bg-emerald-500/70":"bg-red-500/50"}`} style={{width:`${Math.max(prob*100,0.3)}%`}} /></div>
...
function Sec({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
  return (<section className="rounded-lg border border-[color:var(--separator)] bg-[color:var(--surface)] p-3 flex flex-col gap-1.5"><div className="flex items-center gap-1.5"><span className="text-[color:var(--label-tertiary)]">{icon}</span><span className="text-[9px] font-semibold text-[color:var(--label-secondary)] uppercase tracking-wider">{label}</span></div>{children}</section>);
}
...
function Ch({ l, t = "default" }: { l: string; t?: "default"|"ok"|"warn"|"err" }) {
  const c = t==="ok"?"bg-emerald-500/10 text-emerald-400":t==="warn"?"bg-amber-500/10 text-amber-400":t==="err"?"bg-red-500/10 text-red-400":"bg-[color:var(--surface)] text-[color:var(--label-secondary)]";
